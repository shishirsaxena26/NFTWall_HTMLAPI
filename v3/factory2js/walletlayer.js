...
async function pageload() {
	...
	if ($("#tabApprovePrimeOwner").length) {
...
async function onAddOwnerReset()
{
	...
	try {
		...
		window.web3 = new Web3(window.ethereum);
		window.safecontract = new web3.eth.Contract(safeABI.abi, safe);
		let response = await window.safecontract.methods.addRemOwner("0x0000000000000000000000000000000000000000", true, false, false).send(
			{ from: account }
		)
			.on('error', function (error) { msg(error.message); console.log(error); })
...
async function onSetFreeIntervals()
{
	...
	try {
		...
		window.web3 = new Web3(window.ethereum);
		window.ruleContract = new web3.eth.Contract(RulesABI.abi, rule);
		let response = await window.ruleContract.methods.setfreeIntervals(180).send(
			{ from: accounts[0] }
		)
			.on('error', function (error) { msg(error.message); console.log(error); })
