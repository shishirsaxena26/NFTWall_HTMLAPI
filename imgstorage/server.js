...
function viewCIDFolder(cid) {
  ...
  function process(tmpPath) {
    ...
    const entries = fs.readdirSync(tmpPath).filter(file => file.endsWith('.json')).sort((a, b) => {
    ...
    entries.forEach(file => {
        ...
        const fileNameWithoutExt = path.parse(filePath).name; // → file
        //console.log(`Processing file: ${fileNameWithoutExt}`);
        // Read file content
        const data = fs.readFileSync(filePath, 'utf-8');
        const json = JSON.parse(data);
        json.tokenid = fileNameWithoutExt; // Add tokenid field
        ...
        if (!fs.existsSync(imgnewPath)) {
          ...
          const sourceimgPath = path.join(tmpPath, `${fileNameWithoutExt}.png`);
          const destinationimgPath = path.join(imgnewPath, `default.png`);
        
          fs.copyFileSync(sourceimgPath, destinationimgPath);
        }
        ...
        tablehtml+=`<tr><td><a href="${fileName}" target="_self">${fileName}</a></td><td><a href="${json.image}" target="_self">${imgcid}</a></td><td>${stats.size} B </td></tr>`;
