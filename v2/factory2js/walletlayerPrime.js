...
async function pageload() {
	...
	if ($("#txtAdd").length) {	
		...
		debugger;
		onGetPrimeDataOnHome($("#txtAdd").text());
...
async function LoadRulesPrime()
{
	...
	window.PriceABIContract = new web3.eth.Contract(PriceABI.abi, prices);
	
	let ozn2 = await window.PriceABIContract.methods.ozonePriceInUSDT().call();
	$('#oznpricePrime').text($('#oznpricePrime').text() + ` - PriceV2: `+web3.utils.fromWei(ozn2.toString(), 'ether'));
	window.ruleprimeContract = new web3.eth.Contract(RulesABIPrime.abi, rulePrime);
...
async function loadPoolPrime(n, instance, dage) {
	...
	let inc = await window.instancePrimecontract.methods.compute(25).call();
...
async function onMintPrime() {
	...
	try {
		...
		let _value = BigInt(await window.instancePrimecontract.methods.computeMintValue(1).call());
...
async function _loadApprovePrime() {
	...
	await addSafeOwnerPrime("Service for orc1155Prime9Oct", orc1155Prime9Oct);
	await addSafeOwnerPrime("Service for NInstancePrime9Oct", NInstancePrime9Oct);
	await addSafeOwnerPrime("Service(Temp) for ActivateProcess9Oct", ActivateProcess9Oct);
